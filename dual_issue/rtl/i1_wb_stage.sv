...
*/
module i1_wb_stage(input logic [31:0] I1_ALUResultWB, I1_ImmExtWB, I1_PCPlus4WB, PCTargetWB,
                input result_e I1_result_sel,
                input logic clk, mem_read, mem_write, ldst_unsigned,
                input ldst_e ldst_size,
                input logic [31:0] store_data, // rs2
                output logic [31:0] I1_ResultWB, I1_MemResultWB
                );
    ...
    always_comb begin
        case (I1_result_sel)
        RES_ALU: I1_ResultWB = I1_ALUResultWB;
        //RES_MEM: I1_ResultWB = load_dataWB;
        RES_PC4: I1_ResultWB = I1_PCPlus4WB;
        RES_IMM: I1_ResultWB = I1_ImmExtWB;
        RES_PCT: I1_ResultWB = PCTargetWB;
        default: I1_ResultWB = 32'b0;
        endcase
    end

    // Instantiate load/store unit
    lsu lsu(.clk(clk), .mem_read(mem_read), .mem_write(mem_write), .ldst_unsigned(ldst_unsigned), .ldst_size(ldst_size), .addr(I1_ALUResultWB), .store_data(store_data), .load_data(I1_MemResultWB));
